...
return new class extends Migration
{
    ...
    public function up(): void
    {
        ...
        if (!Schema::connection($connection)->hasTable($tableNames['permissions'])) {
            Schema::connection($connection)->create($tableNames['permissions'], static function (Blueprint $table) {
                // $table->engine('InnoDB');
        ...
        if (!Schema::connection($connection)->hasTable($tableNames['roles'])) {
            Schema::connection($connection)->create($tableNames['roles'], static function (Blueprint $table) use ($teams, $columnNames) {
                // $table->engine('InnoDB');
        ...
        if (!Schema::connection($connection)->hasTable($tableNames['model_has_permissions'])) {
            Schema::connection($connection)->create($tableNames['model_has_permissions'], static function (Blueprint $table) use ($tableNames, $columnNames, $pivotPermission, $teams) {
        ...
        if (!Schema::connection($connection)->hasTable($tableNames['model_has_roles'])) {
            Schema::connection($connection)->create($tableNames['model_has_roles'], static function (Blueprint $table) use ($tableNames, $columnNames, $pivotRole, $teams) {
        ...
        if (!Schema::connection($connection)->hasTable($tableNames['role_has_permissions'])) {
            Schema::connection($connection)->create($tableNames['role_has_permissions'], static function (Blueprint $table) use ($tableNames, $pivotRole, $pivotPermission) {
                $table->unsignedBigInteger($pivotPermission);
                $table->unsignedBigInteger($pivotRole);
